...
public class SimpleRatesProvider
  ...
  //-------------------------------------------------------------------------
  /**
   * Converts the type as necessary for testing.
   * 
   * @return the standard implementation
   */
  public ImmutableRatesProvider toImmutableRatesProvider() {
    ImmutableRatesProviderBuilder builder = ImmutableRatesProvider.builder(valuationDate)
        .fxMatrix(FxMatrix.empty());
    if (discountFactors != null) {
      builder.discountFactors(discountFactors);
    }
    if (iborRates != null) {
      builder.iborIndexRates(iborRates);
    }
    if (overnightRates != null) {
      builder.overnightIndexRates(overnightRates);
    }
    if (priceIndexValues != null) {
      builder.priceIndexValues(priceIndexValues);
    }
    return builder.build();
  }

  Curve findCurve(Object root) {
    Iterator<Bean> it = JodaBeanUtils.beanIterator((Bean) root);
    while (it.hasNext()) {
      Bean bean = it.next();
      for (MetaProperty<?> mp : bean.metaBean().metaPropertyIterable()) {
        if (Curve.class.isAssignableFrom(mp.propertyType())) {
          return (Curve) mp.get(bean);
        }
      }
    }
    throw new IllegalStateException("Unable to find curve");
  }

  LocalDateDoubleTimeSeries findTimeSeries(Object root) {
    Iterator<Bean> it = JodaBeanUtils.beanIterator((Bean) root);
    while (it.hasNext()) {
      Bean bean = it.next();
      for (MetaProperty<?> mp : bean.metaBean().metaPropertyIterable()) {
        if (LocalDateDoubleTimeSeries.class.isAssignableFrom(mp.propertyType())) {
          return (LocalDateDoubleTimeSeries) mp.get(bean);
        }
      }
    }
    throw new IllegalStateException("Unable to find curve");
